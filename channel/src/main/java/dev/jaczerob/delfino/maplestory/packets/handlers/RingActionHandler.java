...
public final class RingActionHandler extends AbstractPacketHandler {
    ...
    public static void sendEngageProposal(final Client client, final String name, final int itemid) {
        final int newBoxId = getEngagementBoxId(itemid);
        final Character target = client.getChannelServer().getPlayerStorage().getCharacterByName(name);
        final Character source = client.getPlayer();

        // TODO: get the correct packet bytes for these popups
        if (source.isMarried()) {
            source.dropMessage(1, "You're already married!");
            source.sendPacket(WeddingPackets.getInstance().OnMarriageResult((byte) 0));
            return;
            ...
            source.sendPacket(WeddingPackets.getInstance().OnMarriageResult((byte) 0));
            return;
        } else if (!InventoryManipulator.checkSpace(client, newBoxId, 1, "")) {
        ...
        target.sendPacket(WeddingPackets.getInstance().onMarriageRequest(source.getName(), source.getId()));
    }
    ...
    private synchronized static void breakMarriage(Character chr) {
        ...
        if (partner == null) {
            ...
            partner.sendPacket(WeddingPackets.getInstance().OnNotifyWeddingPartnerTransfer(0, 0));
            resetRingId(partner);
        ...
        chr.sendPacket(WeddingPackets.getInstance().OnNotifyWeddingPartnerTransfer(0, 0));
        resetRingId(chr);
        chr.setPartnerId(-1);
    ...
    public void handlePacket(final InPacket packet, final Client client, final ChannelHandlerContext context) {
        ...
        switch (mode) {
            case 0: // Send Proposal
                sendEngageProposal(client, packet.readString(), packet.readInt());
                break;

            case 1: // Cancel Proposal
                if (client.getPlayer().getMarriageItemId() / 1000000 != 4) {
                    client.getPlayer().setMarriageItemId(-1);
                }
                break;
            ...
            case 2: // Accept/Deny Proposal
                ...
                final Character source = client.getWorldServer().getPlayerStorage().getCharacterByName(name);
                final Character target = client.getPlayer();

                if (source == null) {
                    target.sendPacket(ChannelPacketCreator.getInstance().enableActions());
                    return;
                }
                ...
                if (accepted) {
                    ...
                    if (!InventoryManipulator.checkSpace(client, newItemId, 1, "") || !InventoryManipulator.checkSpace(source.getClient(), newItemId, 1, "")) {
                        target.sendPacket(ChannelPacketCreator.getInstance().enableActions());
                        return;
                    }
                    ...
                    try {
                        ...
                        int marriageId = client.getWorldServer().createRelationship(source.getId(), target.getId());
                        ...
                        InventoryManipulator.addById(client, (newItemId + 1), (short) 1);

                        source.sendPacket(WeddingPackets.getInstance().OnMarriageResult(marriageId, source, false));
                        target.sendPacket(WeddingPackets.getInstance().OnMarriageResult(marriageId, source, false));

                        source.sendPacket(WeddingPackets.getInstance().OnNotifyWeddingPartnerTransfer(target.getId(), target.getMapId()));
                        target.sendPacket(WeddingPackets.getInstance().OnNotifyWeddingPartnerTransfer(source.getId(), source.getMapId()));
                    } catch (Exception e) {
            ...
            case 3: // Break Engagement
                breakMarriageRing(client.getPlayer(), packet.readInt());
                break;

            case 5: // Invite %s to Wedding
                name = packet.readString();
                int marriageId = packet.readInt();
                slot = packet.readByte(); // this is an int

                int itemId;
                try {
                    itemId = client.getPlayer().getInventory(InventoryType.ETC).getItem(slot).getItemId();
                } catch (NullPointerException npe) {
                    client.sendPacket(ChannelPacketCreator.getInstance().enableActions());
                    return;
                }

                if ((itemId != ItemId.INVITATION_CHAPEL && itemId != ItemId.INVITATION_CATHEDRAL) || !client.getPlayer().haveItem(itemId)) {
                    client.sendPacket(ChannelPacketCreator.getInstance().enableActions());
                    return;
                }

                String groom = client.getPlayer().getName();
                String bride = Character.getNameById(client.getPlayer().getPartnerId());
                int guest = Character.getIdByName(name);
                if (groom == null || bride == null || groom.equals("") || bride.equals("") || guest <= 0) {
                    client.getPlayer().dropMessage(5, "Unable to find " + name + "!");
                    return;
                }
                ...
                try {
                    ...
                    if (registration != null) {
                        if (wserv.addMarriageGuest(marriageId, guest)) {
                            ...
                            Channel cserv = client.getChannelServer();
                            ...
                            if (resStatus > 0) {
                                ...
                                Character guestChr = client.getWorldServer().getPlayerStorage().getCharacterById(guest);
                                ...
                                client.getPlayer().dropMessage(5, "Wedding is already under way. You cannot invite any more guests for the event.");
                            }
                        } else {
                            client.getPlayer().dropMessage(5, "'" + name + "' is already invited for your marriage.");
                        }
                    } else {
                        client.getPlayer().dropMessage(5, "Invitation was not sent to '" + name + "'. Either the time for your marriage reservation already came or it was not found.");
                    }
                ...
                client.getAbstractPlayerInteraction().gainItem(itemId, (short) -1);
                break;

            case 6: // Open Wedding Invitation
                slot = (byte) packet.readInt();
                int invitationid = packet.readInt();

                if (invitationid == ItemId.RECEIVED_INVITATION_CHAPEL || invitationid == ItemId.RECEIVED_INVITATION_CATHEDRAL) {
                    Item item = client.getPlayer().getInventory(InventoryType.ETC).getItem(slot);
                    ...
                    Pair<Integer, Integer> coupleId = client.getWorldServer().getWeddingCoupleForGuest(client.getPlayer().getId(), invitationid == ItemId.RECEIVED_INVITATION_CATHEDRAL);
                    if (coupleId != null) {
                        int groomId = coupleId.getLeft(), brideId = coupleId.getRight();
                        client.sendPacket(WeddingPackets.getInstance().sendWeddingInvitation(Character.getNameById(groomId), Character.getNameById(brideId)));
                    }
                }
