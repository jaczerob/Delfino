            ...
            switch (op) {
                case 2: { //put item up for sale
                    ...
                    if (i != null && client.getPlayer().getMeso() >= 5000) {
                        try (Connection con = DatabaseConnection.getStaticConnection();
                            ...
                            if (rs.next()) {
                                if (rs.getInt(1) > 10) { // They have more than 10 items up for sale already!
                                    ...
                                    context.writeAndFlush(getMTS(1, 0, 0));
                                    context.writeAndFlush(ChannelPacketCreator.getInstance().transferInventory(getTransfer(client.getPlayer().getId())));
                                    context.writeAndFlush(ChannelPacketCreator.getInstance().notYetSoldInv(getNotYetSold(client.getPlayer().getId())));
                                    return;
                                }
                            }
                        ...
                        context.writeAndFlush(ChannelPacketCreator.getInstance().MTSConfirmSell());
                        context.writeAndFlush(getMTS(1, 0, 0));
                ...
                case 5: { //change page
                    ...
                    if (tab == 4 && type == 0) {
                        context.writeAndFlush(getCart(client.getPlayer().getId()));
                    } else if (tab == client.getPlayer().getCurrentTab() && type == client.getPlayer().getCurrentType() && client.getPlayer().getSearch() != null) {
                        context.writeAndFlush(getMTSSearch(tab, type, client.getPlayer().getCurrentCI(), client.getPlayer().getSearch(), page));
                    } else {
                        client.getPlayer().setSearch(null);
                        context.writeAndFlush(getMTS(tab, type, page));
                    }
                ...
                case 6: { //search
                    ...
                    context.writeAndFlush(getMTSSearch(tab, type, ci, search, client.getPlayer().getCurrentPage()));
                ...
                case 7: { //cancel sale
                    ...
                    client.enableCSActions();
                    context.writeAndFlush(getMTS(client.getPlayer().getCurrentTab(), client.getPlayer().getCurrentType(),
                    ...
                    try (Connection con = DatabaseConnection.getStaticConnection()) {
                        try (PreparedStatement ps = con.prepareStatement("SELECT * FROM mts_items WHERE seller = ? AND transfer = 1  AND id= ? ORDER BY id DESC")) {
                            ...
                            if (rs.next()) {
                                ...
                                context.writeAndFlush(getMTS(client.getPlayer().getCurrentTab(), client.getPlayer().getCurrentType(),
                                        client.getPlayer().getCurrentPage()));
                                context.writeAndFlush(ChannelPacketCreator.getInstance().MTSConfirmTransfer(i.getQuantity(), i.getPosition()));
                                context.writeAndFlush(ChannelPacketCreator.getInstance().transferInventory(getTransfer(client.getPlayer().getId())));
                            }
                        }
                    } catch (SQLException e) {
                ...
                case 9: { //add to cart
                    ...
                    context.writeAndFlush(getMTS(client.getPlayer().getCurrentTab(), client.getPlayer().getCurrentType(), client.getPlayer().getCurrentPage()));
                    client.enableCSActions();
                    context.writeAndFlush(ChannelPacketCreator.getInstance().enableActions());
                    context.writeAndFlush(ChannelPacketCreator.getInstance().transferInventory(getTransfer(client.getPlayer().getId())));
                    context.writeAndFlush(ChannelPacketCreator.getInstance().notYetSoldInv(getNotYetSold(client.getPlayer().getId())));
                    ...
                    try (Connection con = DatabaseConnection.getStaticConnection();
                        ...
                        if (rs.next()) {
                            ...
                            if (client.getPlayer().getCashShop().getCash(CashShop.NX_PREPAID) >= price) { // FIX
                                ...
                                client.enableCSActions();
                                context.writeAndFlush(getMTS(client.getPlayer().getCurrentTab(), client.getPlayer().getCurrentType(), client.getPlayer().getCurrentPage()));
                                context.writeAndFlush(ChannelPacketCreator.getInstance().MTSConfirmBuy());
                                context.writeAndFlush(ChannelPacketCreator.getInstance().showMTSCash(client.getPlayer()));
                                context.writeAndFlush(ChannelPacketCreator.getInstance().transferInventory(getTransfer(client.getPlayer().getId())));
                                context.writeAndFlush(ChannelPacketCreator.getInstance().notYetSoldInv(getNotYetSold(client.getPlayer().getId())));
                                context.writeAndFlush(ChannelPacketCreator.getInstance().enableActions());
                            } else {
                                context.writeAndFlush(ChannelPacketCreator.getInstance().MTSFailBuy());
                            }
                        }
                    } catch (SQLException e) {
                        e.printStackTrace();
