...
function raiseOpen() {
    ...
    if (questStatus == 0) {
        ...
        const ExpTable = Java.type('dev.jaczerob.delfino.maplestory.constants.game.ExpTable');
        ...
        if (diffExp > 0) {  // thanks IxianMace for noticing Mimiana egg not following progress by EXP
            const ItemInformationProvider = Java.type('dev.jaczerob.delfino.maplestory.server.ItemInformationProvider');
