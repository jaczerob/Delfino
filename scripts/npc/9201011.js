...
var weddingIndoors;
const YamlConfig = Java.type('dev.jaczerob.delfino.maplestory.config.YamlConfig');
        ...
        if (status == 0) {
            ...
            if (state == 0) {    // give player blessings
                ...
                if (YamlConfig.config.server.WEDDING_BLESSER_SHOWFX) {
                    ...
                    target.sendPacket(ChannelPacketCreator.getInstance().showSpecialEffect(9));
                    target.getMap().broadcastMessage(target, ChannelPacketCreator.getInstance().showForeignEffect(target.getId(), 9), false);
                } else {
                            ...
                            case 0:
                                ...
                                if (pid != -1) {
                                    if (pid == player.getId()) {
                                        ...
                                        const RingActionHandler = Java.type('dev.jaczerob.delfino.maplestory.net.server.channel.handlers.RingActionHandler');
