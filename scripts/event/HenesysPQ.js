...
function getEligibleParty(party) {      //selects, from the given party, the team that is allowed to attempt this event
    ...
    return Java.to(eligible, Java.type('dev.jaczerob.delfino.maplestory.net.server.world.PartyCharacter[]'));
}
...
function friendlyItemDrop(eim, mob) {
    if (mob.getId() == 9300061) {
        ...
        mob.getMap().broadcastMessage(ChannelPacketCreator.getInstance().serverNotice(6, "The Moon Bunny made rice cake number " + cakes + "."));
    }
}
...
function friendlyDamaged(eim, mob) {
    if (mob.getId() == 9300061) {
        ...
        if (bunnyDamage > 5) {
            const ChannelPacketCreator = Java.type('dev.jaczerob.delfino.maplestory.tools.ChannelPacketCreator');
            broadcastMessage(ChannelPacketCreator.getInstance().serverNotice(6, "The Moon Bunny is feeling sick. Please protect it so it can make delicious rice cakes."));
