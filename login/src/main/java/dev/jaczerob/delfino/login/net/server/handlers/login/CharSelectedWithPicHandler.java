...
public class CharSelectedWithPicHandler extends AbstractPacketHandler {
    ...
    public void handlePacket(final InPacket p, final Client c) {
        ...
        final var macs = p.readString();
        final var hostString = p.readString();
        ...
        if (c.hasBannedMac() || c.hasBannedHWID()) {
            log.warn("Banned client attempted to login: accountId={}, charId={}, macs={}, hwid={}", c.getAccID(), charId, macs, hwid);
        ...
        if (!server.haveCharacterEntry(c.getAccID(), charId)) {
            log.warn("Client attempted to login to a character that does not belong to their account: accountId={}, charId={}", c.getAccID(), charId);
            SessionCoordinator.getInstance().closeSession(c, true);
            return;
        }
        ...
        if (wserv == null) { // || wserv.isWorldCapacityFull()) {
            log.warn("Client attempted to login to a full world: accountId={}, charId={}, world={}", c.getAccID(), charId, c.getWorld());
