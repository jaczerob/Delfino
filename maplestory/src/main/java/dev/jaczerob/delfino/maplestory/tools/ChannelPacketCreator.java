...
public class ChannelPacketCreator extends PacketCreator {
    ...
    public Packet getPermBan(byte reason) {
    ...
    public Packet getTempBan(long timestampTill, byte reason) {
        OutPacket p = OutPacket.create(SendOpcode.LOGIN_STATUS);
        p.writeByte(2);
        p.writeByte(0);
        p.writeInt(0);
        p.writeByte(reason);
        p.writeLong(getTime(timestampTill));
        return p;
    }

    public Packet getAuthSuccess(Client c) {
        ...
        p.writeBool((YamlConfig.config.server.USE_ENFORCE_ADMIN_ACCOUNT || canFly) && c.getGMLevel() > 1);
        p.writeByte(((YamlConfig.config.server.USE_ENFORCE_ADMIN_ACCOUNT || canFly) && c.getGMLevel() > 1) ? 0x80 : 0);
        p.writeByte(0);
        ...
        p.writeByte(YamlConfig.config.server.ENABLE_PIN && !c.canBypassPin() ? 0 : 1);
    ...
    public Packet getServerList(int serverId, String serverName, int flag, String eventmsg, List<Channel> channelLoad) {
    ...
    public Packet getCharList(Client c, int serverId, int status) {
