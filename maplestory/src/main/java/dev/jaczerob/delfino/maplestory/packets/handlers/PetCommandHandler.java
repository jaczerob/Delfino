...
import dev.jaczerob.delfino.maplestory.client.inventory.PetCommand;
...
public final class PetCommandHandler extends AbstractPacketHandler {
    ...
    public void handlePacket(final InPacket packet, final Client client, final ChannelHandlerContext context) {
        ...
        if (Randomizer.nextInt(100) < petCommand.getProbability()) {
            ...
            chr.getMap().broadcastMessage(ChannelPacketCreator.getInstance().commandResponse(chr.getId(), petIndex, false, command, false));
        } else {
            chr.getMap().broadcastMessage(ChannelPacketCreator.getInstance().commandResponse(chr.getId(), petIndex, true, command, false));
        }
    }
}
