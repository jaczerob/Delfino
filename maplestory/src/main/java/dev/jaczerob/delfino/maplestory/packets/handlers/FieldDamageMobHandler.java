...
public class FieldDamageMobHandler extends AbstractPacketHandler {
    ...
    public RecvOpcode getOpcode() {
        return RecvOpcode.FIELD_DAMAGE_MOB;
    }

    @Override
    public void handlePacket(final InPacket packet, final Client client, final ChannelHandlerContext context) {
        int mobOid = packet.readInt();    // packet structure found thanks to Darter (Rajan)
        ...
        if (map.getEnvironment().isEmpty()) {   // no environment objects activated to actually hit the mob
            log.warn("Chr {} tried to use an obstacle on mapid {} to attack", client.getPlayer().getName(), map.getId());
            return;
        }
        ...
        if (mob != null) {
            if (dmg < 0 || dmg > GameConstants.MAX_FIELD_MOB_DAMAGE) {
                log.warn("Chr {} tried to use an obstacle on mapid {} to attack {} with damage {}", client.getPlayer().getName(),
                        map.getId(), MonsterInformationProvider.getInstance().getMobNameFromId(mob.getId()), dmg);
                return;
            }

            map.broadcastMessage(chr, ChannelPacketCreator.getInstance().damageMonster(mobOid, dmg), true);
