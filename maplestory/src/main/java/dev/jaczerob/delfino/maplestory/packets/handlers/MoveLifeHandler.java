...
public final class MoveLifeHandler extends AbstractMovementPacketHandler {
    ...
    public void handlePacket(final InPacket packet, final Client client, final ChannelHandlerContext context) {
        ...
        byte pNibbles = packet.readByte();
        byte rawActivity = packet.readByte();
        int skillId = packet.readByte() & 0xff;
        int skillLv = packet.readByte() & 0xff;
        short pOption = packet.readShort();
        packet.skip(8);
        ...
        short start_x = packet.readShort(); // hmm.. startpos?
        short start_y = packet.readShort(); // hmm...
        ...
        try {
            ...
            updatePosition(packet, monster, -2);  // Thanks Doodle & ZERO傑洛 for noticing sponge-based bosses moving out of stage in case of no-offset applied
            ...
            map.broadcastMessage(player, ChannelPacketCreator.getInstance().moveMonster(objectid, nextMovementCouldBeSkill, rawActivity, useSkillId, useSkillLevel, pOption, startPos, packet, movementDataLength), serverStartPos);
